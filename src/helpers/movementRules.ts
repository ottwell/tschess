...
export namespace movementRules{
    ...
    export function checkAvailableMoves(piece: gamePiece, game: game): List<number> {
        ...
        var legalBoardMoves2 = checkBlocks(piece, legalBoardMoves1, game.nonCurrentPlayer);
            ...
            case pieceTypes.queen:
                var locations = rawMoves.filter((move) => {
                    if (piece.currentLocation % 8 === 0) {
                        ...
                        if (move === -1 || move === -2 || move === -3 || move === - 4 || (move > 0 && move < 4) || move % 8 === 0 || move === -36 || move === -27 || move === -21 || move === -18 || move === -14 || move === -9 || move === -7 || move === 7 || move === 9 || move === 14 || move === 18 || move === 21 || move === 27 || move === 36)
                            return move;
                    }
                    else if (piece.currentLocation % 8 === 6) {
    ...
    function checkBlocks(piece: gamePiece, locations: List<number>, player: player): List<number> {
        ...
        switch (piece.type) {
            case pieceTypes.pawn:
                ...
                break;
            case pieceTypes.knight:
                res = checkBlockKnight(locations, player);
                break;
            case pieceTypes.rook:
                res = checkBlockRook(piece, locations, player);
                break;
            case pieceTypes.tower:
                res = checkBlockTower(piece, locations, player);
                break;
            case pieceTypes.queen:
    ...
    function initialBlockCheck(piece: gamePiece, locations: List<number>, player: player): List<number>{
        ...
        if(piece.type === pieceTypes.tower){
            let plus8 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 8 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus8 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 8 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let rightSide = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 8 !== 0).OrderBy(m => Math.abs(m - piece.currentLocation)).FirstOrDefault();
            let leftSide = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 8 !== 0).OrderBy(m => Math.abs(m - piece.currentLocation)).FirstOrDefault();
            return new List<number>([minus8, leftSide, rightSide, plus8]).Where(y => y !== undefined);
        }
        else if (piece.type === pieceTypes.rook){
            let plus9 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 9 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus9 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 9 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let plus7 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 7 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus7 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 7 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            return new List<number>([minus9, minus7, plus7, plus9]).Where(y => y !== undefined);
        }
        else if(pieceTypes.queen){
            let plus8 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 8 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus8 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 8 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let rightSide = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0).OrderBy(m =>m).FirstOrDefault();
            let leftSide = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0).OrderByDescending(m =>m).FirstOrDefault();
            let plus9 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 9 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus9 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 9 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let plus7 = locations.Intersect(occupied).Where(t => t - piece.currentLocation > 0 && (t - piece.currentLocation) % 7 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            let minus7 = locations.Intersect(occupied).Where(t => t - piece.currentLocation < 0 && (t - piece.currentLocation) % 7 === 0).OrderBy(x => Math.abs(x - piece.currentLocation)).FirstOrDefault();
            return new List<number>([minus9, minus7, plus7, plus9, minus8, leftSide, rightSide, plus8]).Where(y => y !== undefined).OrderBy(b => b);
        }
    }
    ...
    function checkBlockKnight(locations: List<number>, player: player): List<number>{
        return locations.Except(player.occupiedTiles.Select(z => z.id).ToList());
    }

    function checkBlockRook(piece: gamePiece, locations: List<number>, player: player): List<number> {
        let behindBlocked = new List<number>();
        let blocked = initialBlockCheck(piece, locations, player)
        if (blocked.Count() == 0)
            return locations;
        blocked.ForEach((num) => {
    ...
    function checkBlockTower(piece: gamePiece, locations: List<number>, player: player): List<number> {
        let behindBlocked = new List<number>();
        let blocked = initialBlockCheck(piece, locations, player)
        if (blocked.Count() == 0)
            return locations;
    ...
    function checkBlockQueen(piece: gamePiece, locations: List<number>, player: player): List<number>{
        let behindBlocked = new List<number>();
        let blocked = initialBlockCheck(piece, locations, player)
        if (blocked.Count() == 0)
            return locations;
    ...
    function checkBlockKing(piece: gamePiece, locations: List<number>, player: player): List<number>{
